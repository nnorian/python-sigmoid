...
def task_5(*args, **kwargs):
    integers_list = [arg for arg in args if isinstance(arg, int)]
    strings_list = [key for key, value in kwargs.items() if isinstance(value, str)]
    integers_list.sort()
    strings_list.sort()
...
"""
Task: Creați o funcție cu numele `task_10` care primește un număr variabil
 de argumente de tip integer și un argument `number` de tip integer.
...
def task_13():
    pass
def task_13(base_str, *args):
    base_count = {char: base_str.count(char) for char in base_str}
    for word in args:
        if {char: word.count(char) for char in word} != base_count:
            return False
    return True
# CODUL TĂU VINE MAI SUS:
...
"""
Task: Creați o funcție cu numele `task_16` care va primi un număr variabil de argumente
 de tip integer și un argument `ooperation` de tip string.
Funcția va returna rezultatul operației specificate de argumentul `operation` 
5
aplicată tuturor argumentelor.
...
def task_16(*args, operation):
    if operation == 'add':
        ...
        return "Operația specificată nu este validă!"
    return result
...
Task: Creați o funcție cu numele `task_17` care primește un argument `number`
 după putea primi diferite argumente keyword precum `add`, `sub`, `mul`, `div` 
 care vor fi liste cu numere.
Funcția va returna rezultatul operației specificate de argumentul `operation` 
aplicată tuturor argumentelor. Mai multe operații pot fi aplicate. Ordinea operațiilor
 va fi în ordinea în care sunt specificate.
Operațiile posibile sunt: `add`, `sub`, `mul`, `div`.
...
"""
Task: Creați o funcție cu numele `task_18` care primește un număr variabil 
de argumente de tip string și va returna un dicționar în care cheile vor fi 
caracterele întâlnite în argumentele primite, iar valorile vor fi numărul de
 apariții ale caracterelor.
...
"""
Task: Creați o funcție cu numele `task_20` care primește un număr variabil de argumente de tip string și va returna un dicționar 
în care cheile vor fi lungimile cuvintelor întâlnite în argumentele primite, iar valorile vor fi numărul de apariții ale lungimilor cuvintelor.
